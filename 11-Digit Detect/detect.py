...
def train_evaluate_save_model(model, X_train, y_train, X_test, y_test, num_categories, batch_size=32, name='model', epochs=20):
    """
...
def get_prediction_images(model, folder_name, ext):
    for path in glob.glob(f'{folder_name}/*.{ext}'):
        ...
        grayed = np.average(img_data, axis=2, weights=[.3, .59, .11]) / 255
...
def main():
    ...
    model = create_MNIST_model(input_dim=X_train.shape[1], num_categories=num_categories, name='MNIST-digit-detect')
    
    # Train and evaluate the machine learning model using the training and test data
    hist, loss, categorical_accuracy = train_evaluate_save_model(model, X_train, y_train, X_test, y_test, num_categories, name='MNIST-digit-detect')
    
    # Free up memory
    del MNIST_train_data, MNIST_test_data, X_train, X_test, y_train, y_test, labels, num_categories   
